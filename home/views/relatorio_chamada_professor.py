...
def relatorio_chamada_professor(request):
    ...
    if data_inicio:
        chamadas = chamadas.filter(diario__data_ministrada__gte=data_inicio)

    if data_fim:
        chamadas = chamadas.filter(diario__data_ministrada__lte=data_fim)

    if professor_id:
        chamadas = chamadas.filter(diario__professor_id=professor_id)
    ...
    professores = Docente.objects.filter(ativo=True, escola=escola).order_by("nome")
    ...
    return render(request, "pages/chamada/relatorio_chamada_professor.html", contexto)
